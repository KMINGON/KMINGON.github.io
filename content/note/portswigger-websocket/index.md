...
url = '/note/note-portswigger---websocket-토픽-정리-및-실습/'
+++
...
- **HTTP 헤더를 근거로 한 보안 결정**: `X-Forwarded-For` 처럼 클라이언트가 임의로 넣을 수 있는 헤더로 IP를 판단하면, IP 기반 차단이나 접근 제어가 그대로 우회된다. 헤더·`Referer` 등 클라이언트가 통제하는 값을 접근 제어의 근거로 삼는 문제 전반은 [Access Control Note](/note/note-portswigger---access-control-%ED%86%A0%ED%94%BD-%EC%A0%95%EB%A6%AC-%EB%B0%8F-%EC%8B%A4%EC%8A%B5/)에 정리했다.
