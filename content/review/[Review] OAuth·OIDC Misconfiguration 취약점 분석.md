...
우리는 일상적으로 Google, Microsoft, Kakao 등의 소셜 계정을 통해 다양한 제3자(Third-party) 서비스에 로그인하고 인증한다. 이러한 간편한 인증 및 인가(Authorization)를 가능하게 해주는 핵심 기반 기술이 바로 OAuth 2.0 및 OIDC(OpenID Connect) 프로토콜이다.
...
OIDC(OpenID Connect)는 이 OAuth 2.0 프레임워크 위에 구축된 인증(Authentication) 레이어로, 단순히 '무엇을 할 수 있는지'를 나타내는 Access Token뿐만 아니라, '누가 인증되었는지'를 증명하는 신분증 역할인 ID Token(JWT 기반)을 추가로 발급하여 소셜 로그인을 완성한다.

---

## OAuth/OIDC의 동작 원리 (Grant Type)

OAuth 2.0은 클라이언트의 환경에 따라 권한을 위임받는 다양한 방식(Grant Type)을 제공한다.  
...
마지막으로 클라이언트는 전달받은 코드를 브라우저(프론트엔드)에 노출하지 않고, 백엔드 서버 간 통신을 통해 인가 서버의 Token Endpoint로 전송하여 Access Token(또는 ID Token)과 교환한다.

이 방식은 민감한 토큰이 사용자 브라우저에 직접 노출되지 않기 때문에 PKCE 등장 이전의 표준 방식으로 간주되며, 현재도 자주 쓰이는 방식이다.

### Implicit Grant Type

SPA(Single Page Application)와 같이 백엔드 서버 없이 프론트엔드만 존재하는 공개 클라이언트(Public Client)에 최적화된 방식이다.
![스크린샷 2026-01-13 134435.png](/review/2/image.png)
먼저 앞선 방식과 동일하게 사용자를 인가 서버로 리다이렉트 시킨다.

이후 Authorization Code를 발급하고 이를 다시 교환하는 2단계 과정을 생략하고, 인가 서버가 `redirect_uri`의 URL Fragment(해시 태그 #)에 Access Token을 직접 포함하여 브라우저로 전달한다.
...
앞서 설명한 Implicit Grant Type 에서는 인가 서버(Authorization Server)가 프론트엔드로 AccessToken을 직접 전달한다.

클라이언트(웹 서비스의 백엔드 서버)는 전달받은 토큰을 이용해 인가 서버의 API를 호출하고, 사용자의 식별 정보(보통 Email Claim을 사용한다.)를 받아와 로그인을 처리한다.

이때 개발자가 흔히 저지르는 실수는 `aud` claim 즉, "이 토큰이 정말로 우리 서비스(Client)를 위해 발급된 토큰인가?"를 검증하지 않는 것이다.
...
Salt Labs의 [Oh-Auth — Abusing OAuth to take over millions of accounts](https://salt.security/blog/oh-auth-abusing-oauth-to-take-over-millions-of-accounts) 글에서는 월간 활성 사용자 1억 명 을 보유한 Vidio, 1억 5천만 사용자를 보유한 Bukalapak, 일일 사용자 3천만명을 보유한 Grammarly 등의 서비스에서도 2023년도에 해당 취약점을 발견하였다고 한다.

글쓴이도 2025년 하반기에 진행한 버그바운티에서 3건의 서비스에서 같은 취약점을 발견하여 제보했다.

### 공격 시나리오 분석

그럼 Salt Labs의 사례를 바탕으로 이 취약점이 어떻게 계정 탈취로 이어지는지 구체적인 공격 시나리오를 통해 분석해 보자.

해당 리포트의 타겟 서비스들은 주로 Facebook 로그인을 사용하였으므로, 아래 시나리오는 Facebook Provider를 기준으로 설명한다.
...
하지만 많은 개발자가 이 파라미터의 정확한 방어 원리를 이해하지 못해 정적(Static)인 값을 고정으로 넣거나, 반환된 `state` 값을 아예 검증하지 않는 실수를 범한다.

### 취약점 개요

일반적인 CSRF 공격이 '피해자의 세션(권한)을 이용해 공격자가 원하는 악의적인 행위를 수행하게 만드는 것'이라면, OAuth에서의 Login CSRF는 이와 반대로 '피해자가 공격자의 계정으로 로그인되도록 만드는 현상(Reverse CSRF)'을 의미한다.
...
결과적으로 PKCE의 방어 메커니즘이 무력화(Downgrade)되며, 공격자는 훔친 코드를 이용해 동일하게 Login CSRF(계정 연동) 공격을 성공시킬 수 있다.

---

## 대응 방안

지금까지 살펴본 두 가지 치명적인 취약점(Access Token 주입을 통한 계정 탈취, Login CSRF를 통한 계정 연동 악용)은 모두 프로토콜의 결함이 아닌, 클라이언트(서비스) 구현 단계에서의 '검증 누락'에서 기인한다. 이를 방어하기 위한 핵심 대응 방안은 다음과 같다.
...
인가 코드(Authorization Code)가 전달되는 콜백 엔드포인트는 외부의 공격자가 개입하기 가장 쉬운 지점이다.

State 파라미터의 세션 바인딩 방식으로 인가 요청을 보낼 때 유추할 수 없는 난수(CSRF 토큰)를 생성하여 `state` 파라미터에 담고, 이를 사용자의 브라우저 세션(예: Secure, HttpOnly 속성이 적용된 쿠키)에 안전하게 저장해야 한다.  
...
프로토콜과 인가 서버(Provider) 측의 구현은 점점 더 고도화되고 복잡해지는 반면, 이를 가져다 쓰는 개발자가 직접 수행해야 하는 검증의 책임은 최대한 줄여주는 방향으로 발전하고 있다는 것이다.
...
결국 안전한 OAuth/OIDC 아키텍처를 구축한다는 것은, 단순히 서드파티 Provider의 API를 연동하는 것을 넘어 "우리가 신뢰하는 데이터(Token, Code)가 정말로 우리를 위해, 그리고 현재 요청을 보낸 사용자를 위해 발급된 것이 맞는가?"를 끊임없이 의심하고 검증하는 과정일 것이라는 생각이 든다.
